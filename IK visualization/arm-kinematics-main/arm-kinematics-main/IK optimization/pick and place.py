...
class arm:
    ...
    def draw(self):
        ...
        pygame.draw.line(self.screen, self.colors['black'], (self.Cx, self.Cy), (self.xy0[0], self.xy0[1]), 5)
        pygame.draw.circle(self.screen, self.colors['red'], (self.xy0[0], self.xy0[1]), 8) 
                ...
                if self.active:
                    if event.key == pygame.K_RETURN:
                        ...
                        try:
                            ...
                            pos = [self.x_start, self.y_start, self.obj_height] # here for easy editing
                            self.destination = [(pos[0] + self.Cx),(self.a0 + self.Cy - pos[2])] #vector of 2 for plot
                            
                            self.target_point = [pos[0], pos[1], self.a0-pos[2]] #actual point to be calculated for inverse kinematics
                            print(pos)
                            self.theta0, self.theta1, self.theta2, self.theta3 = self.inverse_kinematics_N4(self.target_point[0], self.target_point[1], self.target_point[2], boundaries=boundaries)
                            ...
                            for j in range(0, (self.top_height - self.obj_height + 1), 20):
                                # time.sleep(0.2)
                                pos = [160, self.end_y, self.obj_height]
                            ...
                            for i in range(0, (self.top_reach - self.x_start + 1), 10):
                                time.sleep(0.2)
                                pos = [self.x_start + i, self.end_y, self.obj_height+j]
                                self.destination = [(pos[0] + self.Cx), (self.a0 + self.Cy - pos[2])]
                            ...
                            time.sleep(2)
                            ser.write(self.str.encode())
                            for k in range((self.top_height - self.obj_height + 1),0, 20):
                                # time.sleep(0.2)
                                pos = [160+i, self.end_y, self.obj_height+j-k]
                                self.destination = [(pos[0] + self.Cx), (self.a0 + self.Cy - pos[2])]
                                self.target_point = [pos[0], pos[1], self.a0 - pos[2]]
                                print(pos)
                                self.theta0, self.theta1, self.theta2, self.theta3 = self.inverse_kinematics_N4(
